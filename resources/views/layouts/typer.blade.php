<div class="typerPos text-sm-center">
    <h2>Hey! I'm Šarūnas</h2>
    <h1>

        <span class="typerSize">In love with </span> <span
                class="typer"
                id="some-id"
                data-words="Web!, Elegant code!, Developing stuff!, PHP!,"
                data-delay="100"
                data-colors="#0011FF,#177E89,#8B0000,#81FF00,">
  </span>

        <span style="font-size:1.2em;vertical-align:middle;" class="cursor" data-cursorDisplay="|"
              data-owner="some-id"></span>
    </h1>

    <div class="typerButton"><i class="fas fa-chevron-down"></i></div>
</div>